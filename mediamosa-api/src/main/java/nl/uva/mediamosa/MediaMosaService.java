...
public class MediaMosaService extends Service {
    ...
    //new
    public Response uploadStill(String uploadHost, String assetId, Map map, InputStream stream, String mimeType, String filename) throws IOException, ServiceException {
        prepare();
        return vpcoreImpl.uploadStill(uploadHost, assetId, map, stream, mimeType, filename);
    }
            ...
            try {
                ...
                if (!(impl instanceof MediaMosa)) {
                    throw new InstantiationException(
                            "Implementation class is null or not an instance of MediaMosa");
                }
            } catch (Exception e) {
                String msg = e.getMessage() + " - " + e.getCause();
                logger.error("Error occurred while loading MediaMosa service", e);
